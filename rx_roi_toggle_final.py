...
# Modo de búsqueda de fiduciales:
#   AUTO_CENTER: primero busca en el centro y, si falla, usa toda la imagen.
#   CENTER:      solo busca en la región central.
#   FULL:        comportamiento original; busca en toda la imagen.
DETECTION_MODE = "AUTO_CENTER"
CENTER_ROI_FRACTIONS = (0.70, 0.85)
VALID_DETECTION_MODES = {"AUTO_CENTER", "CENTER", "FULL"}
...
def parse_runtime_args(default_modulation: str, default_detection_mode: str):
    """
    Permite ejecutar, sin romper compatibilidad:
        python rx.py OOK_MANCHESTER
        python rx.py ASK4_GRAY
        python rx.py CSK_RGB
        python rx.py OOK_MANCHESTER CENTER
        python rx.py OOK_MANCHESTER FULL
        python rx.py OOK_MANCHESTER CENTER mensaje.txt

    Modos de búsqueda:
        CENTER      -> busca fiduciales solo en el centro.
        FULL        -> busca en toda la imagen, como antes.
        AUTO_CENTER -> primero centro y luego imagen completa si no encuentra.
    """
    rx_mode = default_modulation
    ...
    for raw_arg in sys.argv[1:]:
        ...
        if upper in VALID_RX_MODULATIONS:
            rx_mode = upper
        elif upper in detection_aliases:
            detection_mode = detection_aliases[upper]
        else:
            text_path = token

    if rx_mode not in VALID_RX_MODULATIONS:
        raise ValueError(
            f"Modo RX inválido: {rx_mode}. "
            f"Opciones válidas: {sorted(VALID_RX_MODULATIONS)}"
        )

    if detection_mode not in VALID_DETECTION_MODES:
        raise ValueError(
            f"Modo de búsqueda inválido: {detection_mode}. "
            f"Opciones válidas: {sorted(VALID_DETECTION_MODES)}"
        )

    return rx_mode, detection_mode, text_path


def load_expected_text_from_path(default_text: str, path: str | None) -> str:
    """
...
# ─────────────────────────── CLASE RECEPTOR ──────────────────────────────────
class Rx:
    def __init__(self, scale=PROCESS_SCALE, warp_size=WARP_SIZE, modulation="AUTO", detection_mode=DETECTION_MODE):
        self.scale = scale
    ...
    def reset(self, announce=True):
        ...
        self._last_search_area = ""
        self._last_fid_count = 0
    ...
    def open_camera(self, cam_id=0):
        ...
        print(f"Búsqueda:   {self.detection_mode}  (CENTER/FULL/AUTO_CENTER)")
    ...
    # ── Búsqueda de fiduciales: centro o imagen completa ─────────────────────
    def _center_crop(self, frame, fraction: float):
        h, w = frame.shape[:2]
    ...
    def _candidate_rois(self, frame):
        """Devuelve regiones donde se intentará detectar fiduciales."""
        if self.detection_mode == "FULL":
            return [("FULL", frame, np.array([0, 0], dtype=np.float32))]

        if self.detection_mode == "CENTER":
            frac = CENTER_ROI_FRACTIONS[0]
            roi, offset = self._center_crop(frame, frac)
            return [(f"CENTER_{int(frac * 100)}", roi, offset)]

        # AUTO_CENTER: primero centro, luego un centro más amplio y al final imagen completa.
        candidates = []
        for frac in CENTER_ROI_FRACTIONS:
            roi, offset = self._center_crop(frame, frac)
            candidates.append((f"CENTER_{int(frac * 100)}", roi, offset))

        candidates.append(("FULL", frame, np.array([0, 0], dtype=np.float32)))
        return candidates

    def _detect_fiducials_in_view(self, view):
        scaled = cv2.resize(
            view,
            None,
            fx=self.scale,
        ...
        return detect_fiducials(bw, contours, hierarchy)
    ...
    def process_frame(self, frame):
        ...
        # 1) Intentar búsqueda según modo seleccionado: CENTER, FULL o AUTO_CENTER.
        for area_name, view, offset in self._candidate_rois(frame):
            fiducials = self._detect_fiducials_in_view(view)
            best_fid_count = max(best_fid_count, len(fiducials))

            if len(fiducials) >= 4:
                selected = (area_name, fiducials, offset)
                break

        self._last_fid_count = best_fid_count

        if selected is None:
            self._last_search_area = self.detection_mode
            self._draw_hud(debug, fid_count=best_fid_count)
            return debug, None, self._decoded_text

        area_name, fiducials, offset = selected
        self._last_search_area = area_name

        # 2) Dibujar fiduciales encontrados, ya convertidos a coordenadas de la imagen completa.
        for f in fiducials:
        ...
        cell_means, color_means, binary_bits, calibration = read_symbol_cells(
            warp_gray,
            warp_bgr=warp,
            border=0
        )
    ...
    def _draw_hud(self, debug, fid_count=0, just_processed=False):
        ...
        if self._last_search_area:
            cv2.putText(
                debug,
                f"busqueda={self.detection_mode} / usado={self._last_search_area}",
                (10, 60),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.48,
                (255, 180, 0),
        ...
        event_y = 90 if self._last_search_area else 60
...
if __name__ == "__main__":
    rx_mode, detection_mode, text_path = parse_runtime_args(RX_MODULATION, DETECTION_MODE)
    EXPECTED_TEXT = load_expected_text_from_path(EXPECTED_TEXT, text_path)
    ...
    print(f"Modo de búsqueda de fiduciales: {detection_mode}")
    ...
    print("Para activar/desactivar búsqueda central:")
    print("  python rx.py OOK_MANCHESTER CENTER")
    print("  python rx.py OOK_MANCHESTER FULL")
    print("  python rx.py OOK_MANCHESTER AUTOCENTER")
    print("=" * 70)
    print("")
